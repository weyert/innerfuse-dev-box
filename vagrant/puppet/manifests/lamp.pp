# Kick-start the installation of the LAMP environment by updating
# the package repositories to the latest state
include apt_update
...
# Include the project configuration parameters
include params
...
class php5 {
    ...
    file { "php-composer.ini":
        path => "/etc/php5/cli/conf.d/40-composer.ini",
        ensure => file,
        content => template('default/php-composer.ini'),
        require => Package["php5-cli"]
    }    
}
...
#    }    
    ...
    # PHP_CodeSniffer
#    package { "pear-pear.php.net-PHP_CodeSniffer":
...
#    }
    ...
    # Phing
#    package { "pear-pear.php.net-Phing":
#        name => "Phing",
#        source => "pear.php.net/Phing",
#        ensure => "latest",
#        require => Package["php-pear"],
#    }
}
...
class composer {
    # Only run the composer application when we need to install Symfony2
    if $params::symfony == true {
        exec { "composerPhar":
            cwd => "/vagrant",
